...
export default function Home() {
  ...
  useEffect(() => {
    ...
    wsRef.current.onerror = (error) => {
      ...
      setMsgs((prev) => [...prev, { mode: "info", data: "WebSocket error: " + JSON.stringify(error) }]);
    }
    wsRef.current.onopen = () => {
      setMsgs((prev) => [...prev, { mode: "info", data: "WebSocket connected" }]);
    };
    wsRef.current.onmessage = (event) => {
      setMsgs((prev) => [...prev, { mode: "recv", data: JSON.stringify(event.data) }]);
    };
    return () => {
      wsRef.current?.close();
      wsRef.current = null;

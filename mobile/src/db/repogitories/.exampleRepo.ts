import { createBaseRepository } from "./_baseRepo";
import { db } from "../index";
import { examples } from "../schema/.exampleSchema";
import { eq } from "drizzle-orm";

export const exampleRepo = {
  ...createBaseRepository(db, examples),

  // You can add custom methods specific to the example table here
  additionalLogic: async (param: number) => {
    const result = await db.select().from(examples).where(eq(examples.createdAt, param)).get();
    return result;
  }
};


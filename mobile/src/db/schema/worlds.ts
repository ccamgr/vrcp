...
export const worlds = sqliteTable("worlds", {
  ...
  rawData: text("raw_data", { mode: 'json' }).notNull().$type<World>(),
});

export type DBWorld = typeof worlds.$inferInsert;

export function convertToDBWorld(world: World, favoriteGroupId?: string | null): DBWorld {
  return {
    id: world.id,
    name: world.name,
...
export function convertFromDBWorld(dbWorld: DBWorld): World {
  return dbWorld.rawData;
}

...
export default function HistoryTimeline({
  ...
  return (
    <View style={styles.timelineContainer}>
      <ScrollView
        contentContainerStyle={{ height: DAY_HEIGHT + 50, paddingBottom: 50 }}
      >
        <View style={styles.timelineInnerVertical}>
          ...
          {timeMarkers.map((marker, index) => {
            ...
            return (
              <View
                key={index}
                style={[styles.gridLineContainer, { top: `${topPercent}%` }]}
              >
                <Text style={styles.timeLabelVertical}>{marker.label}</Text>
          ...
          <View style={styles.sessionLayer}>
            {sessions.map((session, index) => {
              ...
              const minHeightPercent =
                (MIN_SESSION_HEIGHT_MS / totalDuration) * 100; // 最低1分相当
      ...
      {selectedSession && (
        <View
          style={[styles.detailOverlay, { backgroundColor: theme.colors.card }]}
        >
          ...
          <Text style={{ color: theme.colors.text, fontSize: 12 }}>
            ...
            {Math.floor(selectedSession.durationMs / 60000)} min)
          </Text>
          <Text

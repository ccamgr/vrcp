...
export default function InstanceDetail() {
  ...
  const owner = useMemo(() => {
    if (isUserOwner && userOwner)
      return { type: "user" as const, data: userOwner };
    if (isGroupOwner && groupOwner)
      return { type: "group" as const, data: groupOwner };
    return null;
      ...
      {instance ? (
        <View style={{ flex: 1 }}>
          ...
          >
            {/* Friends in Instance */}
            <DetailItemContainer
              title={t("pages.detail_instance.sectionLabel_usersInInstance")}
            >
              <View style={styles.detailItemContent}>
                {friendsInInstance.map((friend) => (
                  ...
                  >
                    <UserOrGroupChip
                      data={friend}
                      textColor={getTrustRankColor(friend, true, false)}
                    />
                  </TouchableEx>
                ))}
                {instance.n_users > friendsInInstance.length && (
                  <Text style={[styles.moreUser, { color: theme.colors.text }]}>
                    {t(
                      "pages.detail_instance.section_users_more_user_count_other",
                      {
            ...
            {/* World Info */}
            <DetailItemContainer
              title={t("pages.detail_instance.sectionLabel_world")}
            >
              <View style={styles.detailItemContent}>
                <TouchableEx
                  style={styles.horizontal}
                  onPress={() => routeToWorld(instance.worldId)}
                >
                  <CachedImage
                  ...
                  />
                  <Text
                    style={[styles.worldName, { color: theme.colors.text }]}
                  >
                    {instance.world.name}
                  </Text>
                </TouchableEx>
            ...
            {owner && (
              <DetailItemContainer
                title={t("pages.detail_instance.sectionLabel_owner")}
              >
                <View style={styles.detailItemContent}>
                  <TouchableEx
                    onPress={() =>
                      owner.data?.id &&
                      (owner.type === "user"
                        ? routeToUser(owner.data.id)
                        : routeToGroup(owner.data.id))
                    }
                  >
                    <UserOrGroupChip
                      data={owner.data!}
                      ...
                      textColor={
                        owner.type === "user"
                          ? getTrustRankColor(owner.data as any, true, false)
                          : undefined
                      }
                    />
                  </TouchableEx>
            ...
            {/* Platforms & Tags */}
            <DetailItemContainer
              title={t("pages.detail_instance.sectionLabel_platform")}
            >
              <View style={styles.detailItemContent}>
            ...
            <DetailItemContainer
              title={t("pages.detail_instance.sectionLabel_tags")}
            >
              <View style={styles.detailItemContent}>
                <TagChips
                  tags={getAuthorTags(instance.world)}
            ...
            {/* Basic Info */}
            <DetailItemContainer
              title={t("pages.detail_instance.sectionLabel_info")}
            >
              <View style={styles.detailItemContent}>
                <Text style={{ color: theme.colors.text }}>
                  {t("pages.detail_instance.section_info_capacity", {
                    capacity: instance.capacity,
                  })}
                </Text>
                <Text style={{ color: theme.colors.text }}>
                  {t("pages.detail_instance.section_info_ageGated", {
                    ageGated: instance.ageGate,
                  })}
                </Text>
              </View>
            </DetailItemContainer>

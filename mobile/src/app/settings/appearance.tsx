import SettingItem, {
  SettingItemProps,
} from "@/components/features/settings/SettingItem";
import ThemeModal, {
  getIconName as getIconNameCS,
} from "@/components/features/settings/appearance_innermodals/ThemeModal";
import HomeTabLayoutModal, {
  getIconName as getIconNameHT,
} from "@/components/features/settings/appearance_innermodals/HomeTabLayoutModal";
import CardViewColumnsModal, {
  getIconName as getIconNameCV,
} from "@/components/features/settings/appearance_innermodals/CardViewColumnsModal";
...
export default function AppearanceSettings() {
  ...
  const [colorSchemaModal, setColorSchemaModal] = useState<
    InnerModalOption<typeof settings.uiOptions_colorSchema>
  >({ open: false });
  const [homeTabModeModal, setHomeTabModeModal] = useState<
    InnerModalOption<{
  ...
  >({ open: false });
  const [cardViewColumnsModal, setCardViewColumnsModal] = useState<
    InnerModalOption<typeof settings.uiOptions_cardViewColumns>
  >({ open: false });
  const [friendColorModal, setFriendColorModal] = useState<
    InnerModalOption<typeof settings.uiOptions_friendColor>
  >({ open: false });
  const [favoriteFriendsColorsModal, setFavoriteFriendsColorsModal] = useState<
    InnerModalOption<typeof settings.uiOptions_favoriteFriendsColors>
  >({ open: false });
  ...
  const listContents: SettingItemListContents = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          description: t(
            "pages.setting_appearance.itemDescription_homeTabLayout",
          ),
          leading: (
            <IconSymbol
          ...
          onPress: () =>
            setHomeTabModeModal({
              ...
              onSubmit: (value) => {
                saveSettings({
                  uiOptions_homeTabTopVariant:
                    value.top || settings.uiOptions_homeTabTopVariant,
                  uiOptions_homeTabBottomVariant:
                    value.bottom || settings.uiOptions_homeTabBottomVariant,
                  uiOptions_homeTabSeparatePos:
                    value.sepPos || settings.uiOptions_homeTabSeparatePos,
                });
              },
            }),
        },
        {
          ...
          description: t(
            "pages.setting_appearance.itemDescription_cardViewColumns",
          ),
          leading: (
            <IconSymbol
      ...
      items: [
        {
          ...
          description: t(
            "pages.setting_appearance.itemDescription_friendColor",
          ),
          leading: (
            <ColorSquarePreview colors={[settings.uiOptions_friendColor]} />
          ),
          onPress: () =>
            setFriendColorModal({
              open: true,
        ...
        },
        {
          ...
          description: t(
            "pages.setting_appearance.itemDescription_favoriteFriendsColors",
          ),
          leading: (
            <ColorSquarePreview
              colors={Object.values(settings.uiOptions_favoriteFriendsColors)}
            />
          ),
          onPress: () =>
            setFavoriteFriendsColorsModal({
              open: true,
  ...
  return (
    <GenericScreen scrollable>
      ...
      <CardViewColumnsModal
        ...
        setOpen={(v) =>
          setCardViewColumnsModal((prev) => ({ ...prev, open: v }))
        }

  ...
  const getEventContent = (event: VrcLogEvent) => {
    switch (event.type) {
      ...
      case "Login":
        return {
          color: "text-blue-400",
          text: `Login: ${event.data.username}`,
        };
      case "WorldEnter":
        return {
          color: "text-yellow-400",
          text: `World: ${event.data.world_name}`,
        };
      case "InstanceJoin":
        return {
          color: "text-orange-400",
          text: `Instance: ${event.data.instance_id}`,
        };
      case "PlayerJoin":
        return {
          color: "text-cyan-400",
          text: `[+] ${event.data.player_name}`,
        };
      case "PlayerLeft":
        return {
          color: "text-gray-400",
    ...
    <div className="flex flex-col h-full">
      <header className="flex justify-between items-center p-6">
        ...
        <div className="flex gap-4 text-sm text-slate-400">
          ...
          <button onClick={clearLogs} className="hover:text-red-400 transition">
            Clear
          </button>
        </div>
      </header>
          ...
          logs.map((log, index) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`font-bold mr-3 w-28 shrink-0 text-xs py-0.5 ${color}`}
                >
                  {log.event.type}
                </span>
